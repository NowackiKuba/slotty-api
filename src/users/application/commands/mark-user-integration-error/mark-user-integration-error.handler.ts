...
import { MarkUserIntegrationErrorCommand } from './mark-user-integration-error.command';

@CommandHandler(MarkUserIntegrationErrorCommand)
export class MarkUserIntegrationErrorHandler implements ICommandHandler<
  MarkUserIntegrationErrorCommand,
...
> {
  ...
  async execute(
    command: MarkUserIntegrationErrorCommand,
  ): Promise<UserIntegrationReadModel> {
    const { message, provider, userId } = command.payload;
    ...
    integration.markAsError(message);
